connect_quant <- function() {
  DBI::dbConnect(RSQLite::SQLite(), "sql/quant-db.sqlite")
}
...
fill_database <- function() {
  ...
  if (nrow(sp50) > 0) {
    ...
    if (!("quant_meta" %in% DBI::dbListTables(mydb))) {
      DBI::dbWriteTable(mydb, "quant_meta", sp50)
    } else {
      DBI::dbAppendTable(mydb, "quant_meta", sp50)
    }
