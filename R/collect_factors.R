get_complete_factor_list <- function(){
  qrs_out <- dplyr::tbl(connect_quant(), "qrs_data") %>%
    ...
    dplyr::select(date,symbol,adj_close,one_month_fwd,everything()) %>%
  ...
  key_join <- qrs_out %>%
    ...
    dplyr::mutate_at(dplyr::vars(group,factor),funs(snakecase::to_title_case))
  ...
  normal_val <- qrs_out %>%
    ...
    dplyr::mutate(normalized_value = dplyr::ntile(value,n = 5),
...
plot_alpha_historical_perf <- function(complete_factor_list,from_date = "2020-01-01"){
  complete_factor_list %>%
    ...
    dplyr::group_split() %>%
    purrr::map(~{
      highcharter::hchart(.x,"line",hcaes(date,cum_ret*100,group = normalized_value)) %>%
        ...
        highcharter::hc_add_series(get_market_data() %>% dplyr::mutate(
          ...
          dplyr::filter(date >= lubridate::as_date(from_date)) %>%
          dplyr::mutate(cum_ret = dplyr::with_order(date,cumprod,1+one_month_fwd)-1),
        "line",hcaes(date,cum_ret*100),name = "S&P 500") %>%
